...
const SERVICES_DIR = Deno.env.get("SERVICES_DIR") || "~/projects/services";
...
Deno.serve({
  ...
  if (!body.service || !body.tag) {
    await logger.error(
      `Missing required fields (400): ${JSON.stringify({ body }, null, 2)}`,
    );
  ...
  if (!ACCEPTED_SERVICES.includes(body.service)) {
    await logger.error(
      `Invalid service requested (400): ${body.service}. Accepted services: ${
        ACCEPTED_SERVICES.join(", ")
      }`,
    );
    return new Response(
      `Service must be one of ${ACCEPTED_SERVICES.join(", ")}`,
      { status: 400 },
    );
  }
  ...
  if (!ACCEPTED_TAGS.includes(body.tag)) {
    await logger.error(
      `Invalid tag requested (400): ${body.tag}. Accepted tags: ${
        ACCEPTED_TAGS.join(", ")
      }`,
    );
    return new Response(
      `Tag must be one of ${
        ACCEPTED_TAGS.join(", ")
      }. Did you forget to update the code and redeploy it?`,
      { status: 400 },
    );
  }
  ...
  // Pull docker image
  const image = new Deno.Command(
    `docker pull ghcr.io/robertjshirts/${body.service}:${body.tag}`,
  );
  ...
  // Run docker compose
  const compose = new Deno.Command(
    `docker compose -f ${composeFile} restart ${body.service}`,
  );
  ...
  if (composeStatus.code !== 0) {
    ...
    await logger.error(
      `Docker compose restart failed (500):\nError: ${errorOutput}`,
    );
